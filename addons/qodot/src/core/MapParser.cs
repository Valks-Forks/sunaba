		...
		switch (scope)
		{
			...
			case ParseScope.PROPERTY_VALUE:
				...
				if (isLast)
				{
					...
					if (!currentEntity.properties.ContainsKey(propKey))
						currentEntity.properties.Add(propKey, prop);
					else
						currentEntity.properties[propKey] = prop;
					scope = ParseScope.ENTITY;
				}
