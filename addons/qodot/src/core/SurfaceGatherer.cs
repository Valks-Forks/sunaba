...
public class SurfaceGatherer
{
	...
	private bool FilterBrush(int entityIdx, int brushIdx)
	{
		...
		for (int f = 0; f < faceSpan.Length; f++)
		{
			...
			{
				if (faceSpan[f].textureIdx == mapData.worldspawnLayers[l].textureIdx)
	...
	private bool FilterFace(int entityIdx, int brushIdx, int faceIdx)
	{
		...
		if (faceGeoSpan[faceIdx].vertices.Count < 3)
			return true;

		// Omit faces textured with skip.
		if (faceFilterTextureIdx != -1 && faceSpan[faceIdx].textureIdx == faceFilterTextureIdx)
			return true;

		// Omit filtered texture indices.
		if (textureFilterIdx != -1 && faceSpan[faceIdx].textureIdx != textureFilterIdx)
			return true;
