...
public partial class Player : CharacterBody3D
{
	...
	public override void _PhysicsProcess(double delta)
	{
		...
		if (((Velocity.Length() == 0) || (vel.Length() == 0)) && !Input.IsActionPressed("crouch"))
		{
			...
			if (!isReloading && gunAnimationPlayer.CurrentAnimation == "fire")
				gunAnimationPlayer.Play("idle");
		}
		...
		else
		{
			...
			else if (speed == sprintSpeed && IsOnFloor())
			{
				...
				if (!isReloading && gunAnimationPlayer.CurrentAnimation == "fire")
					gunAnimationPlayer.Play("idle_2");
			}
			else if (!IsOnFloor())
			{
		...
		if ((!IsMultiplayerAuthority() && global.isNetworkedGame) || Input.MouseMode == Input.MouseModeEnum.Visible)
			return;
